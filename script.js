...
function showTab(panel, tab) {
  ...
  if (panel === 'joke') {
    document.getElementById('joke-xhr-tab').style.display = tab === 'xhr' ? 'block' : 'none';
    document.getElementById('joke-fetch-tab').style.display = tab === 'fetch' ? 'block' : 'none';
  }
}

// ── Logging ──
function addLog(type, msg, meta) {
  const entry = { type, msg, meta, time: new Date().toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',second:'2-digit'}) };
  logEntries.unshift(entry);
  if (type === 'ok') logOk++; else if (type === 'err') logErr++;
  renderLog();
}
...
function renderLog() {
  ...
  if (!logEntries.length) {
    el.innerHTML = '<div style="color:var(--muted);font-size:13px;text-align:center;padding:20px">No activity yet.</div>';
    return;
  }
...
function setStatus(id, code, label) {
  ...
  el.innerHTML = `<span class="status-badge ${ok ? 'ok' : 'err'}">${ok ? '●' : '✕'} ${code} ${label}</span>`;
}

// ══════════════════════════════════════════
// 1. WEATHER  (async/await + Geocoding API)
// ══════════════════════════════════════════
async function fetchWeather() {
  ...
  try {
    ...
    const codes = {0:'Clear sky',1:'Mainly clear',2:'Partly cloudy',3:'Overcast',45:'Foggy',48:'Icy fog',51:'Light drizzle',53:'Moderate drizzle',61:'Slight rain',63:'Moderate rain',71:'Slight snow',80:'Rain showers',95:'Thunderstorm'};
    ...
    document.getElementById('w-feels').textContent = `${c.apparent_temperature}°C`;
    document.getElementById('w-cloud').textContent = `${c.cloud_cover}%`;
    document.getElementById('weather-result').style.display = 'block';

    addLog('ok', `Weather fetched: ${name}, ${country}`, `${c.temperature_2m}°C · ${desc}`);
  } catch (e) {
...
function fetchJoke() {
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === XMLHttpRequest.DONE) {
      ...
      document.getElementById('joke-xhr-status').innerHTML = `<span class="status-badge ${ok ? 'ok' : 'err'}">● ${xhr.status}</span>`;
      try {
  ...
  fetch(url)
    ...
    .then(({ d, status }) => {
      ...
      document.getElementById('joke-fetch-status').innerHTML = `<span class="status-badge ${ok ? 'ok' : 'err'}">● ${status}</span>`;
...
async function fetchDog() {
  ...
  try {
    ...
    img.onload = () => {
      ...
      document.getElementById('dog-url').textContent = data.message.substring(0, 55) + '…';
    };
...
// ══════════════════════════════════════════
// 4. QUOTE (.then chaining)
// ══════════════════════════════════════════
function fetchQuote() {
  const tag = document.getElementById('quote-tag').value;
  const url = `https://api.quotable.io/random${tag ? '?tags=' + tag : ''}`;
  spin('quote-spin', true);
  document.getElementById('quote-display').style.display = 'none';
  setBox('quote-raw', `▸ Step 1: Calling fetch("${url}")\n`, 'loading');

  fetch(url)
    .then(res => {
      const box = document.getElementById('quote-raw');
      box.innerHTML += `▸ Step 2: Response received — Status ${res.status}\n`;
      return res.json();
    })
    .then(data => {
      const box = document.getElementById('quote-raw');
      box.innerHTML += `▸ Step 3: JSON parsed successfully\n\n`;
      box.innerHTML += colorize(data);
      ...
      tagsEl.innerHTML = (data.tags || []).map(t =>
        `<span style="background:var(--surface2);border:1px solid var(--border);font-size:11px;padding:2px 8px;border-radius:4px;color:var(--muted)">${t}</span>`
      ).join('');
      document.getElementById('quote-display').style.display = 'block';

      document.getElementById('quote-status').innerHTML = `<span class="status-badge ok">● 200 OK</span>`;
      addLog('ok', `Quote fetched — ${data.author}`, data.content.substring(0, 60) + '…');
    })
    .catch(err => {
      setBox('quote-raw', `Error: ${err.message}`, 'error');
      document.getElementById('quote-status').innerHTML = `<span class="status-badge err">✕ Error</span>`;
      addLog('err', 'Quote API failed', err.message);
    })
    .finally(() => spin('quote-spin', false));
}

// ══════════════════════════════════════════
// 5. IP Geolocation (Promise.all)
// ══════════════════════════════════════════
async function fetchIP() {
  const ipInput = document.getElementById('ip-input').value.trim();
  const target = ipInput || '';
  spin('ip-spin', true);
  setBox('ip-raw', 'Running parallel requests with Promise.all()…', 'loading');
  document.getElementById('ip-result').style.display = 'none';

  try {
    const [res1] = await Promise.all([
      fetch(`http://ip-api.com/json/${target}?fields=status,country,regionName,city,zip,lat,lon,timezone,isp,org,as,query`)
    ]);

    const data = await res1.json();
    setBox('ip-raw', colorize(data), 'success');
    setStatus('ip-status', 200, 'OK');

    const fields = ['query','city','regionName','country','zip','lat','lon','timezone','isp','org','as'];
    const grid = document.getElementById('ip-grid');
    grid.innerHTML = fields.map(k => data[k] !== undefined ? `
      <div class="weather-stat">
        <div class="weather-stat-label">${k}</div>
        <div class="weather-stat-value" style="font-size:13px;word-break:break-all">${data[k] || '—'}</div>
      </div>` : '').join('');
    document.getElementById('ip-result').style.display = 'block';
    addLog('ok', `IP lookup: ${data.query}`, `${data.city}, ${data.country}`);
  } catch (e) {
    setBox('ip-raw', `Error: ${e.message}\n\nNote: ip-api.com may be blocked on HTTPS pages. Try in an HTTP context or use CORS proxy.`, 'error');
    addLog('err', 'IP API failed', e.message);
  }
  spin('ip-spin', false);
}

// ══════════════════════════════════════════
// 6. CURRENCY (Frankfurter API)
// ══════════════════════════════════════════
async function fetchCurrency() {
  ...
  try {
    ...
    document.getElementById('fx-converted').textContent = `${converted.toFixed(2)} ${to}`;
    document.getElementById('fx-rate').textContent = `1 ${from} = ${(converted/amount).toFixed(4)} ${to} · Date: ${data.date}`;
    document.getElementById('fx-result').style.display = 'block';

    addLog('ok', `FX: ${amount} ${from} → ${converted.toFixed(2)} ${to}`, `Rate: ${(converted/amount).toFixed(4)}`);
  } catch (e) {
...
async function simulateWebhook() {
  ...
  try {
    ...
    addLog('ok', `Webhook fired: ${event}`, `Status: ${res.status} · id: ${resp.id}`);
  } catch (e) {
...
function receiveWebhook(event, body, status) {
  ...
  entry.innerHTML = `
    ...
    <div style="font-size:10px;color:var(--muted);margin-top:4px;font-family:var(--mono)">${t} · ${body.timestamp}</div>
  `;
  log.insertBefore(entry, log.firstChild);
}

function clearWebhookLog() {
  document.getElementById('wh-log').innerHTML = '<div style="color:var(--muted);font-size:12px;padding:12px 0;text-align:center">Waiting for events…</div>';
  whCount = 0;
...
function updatePostPreview() {
  ...
  if (el) el.innerHTML = `<span class="key">POST</span> https://jsonplaceholder.typicode.com/posts\nContent-Type: <span class="str">${ct}</span>\n\n${colorize(payload)}`;
}
